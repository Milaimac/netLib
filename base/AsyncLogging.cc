...
void AsyncLogging::threadFunc(){
    ...
    while(running_){
...

        {
            ...
            if(buffers_.empty()){ //如果buffers_.empty()就说明currentBuffer没有放入buffers_,也就是前端写入数据极少
                cond_.waitForSeconds(flushInterval_);
            }
            buffers_.push_back(std::move(currentbuffer_));  //等待之后将当前buffer放入交付给后端
            bufferToWrite.swap(buffers_);
            if(!nextbuffer_){           //如果nextBuffer已经传给了currentBuffer
        ...
        if(bufferToWrite.size() > 25){      // 日志数据过多，buffer本来reserve16个的，现在是25个， 直接把bufferToWrite.begin()+2, 到end的全部删掉
            char buf[256];
