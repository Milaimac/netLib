...
// suffix 是后缀的意思， 通过后缀得到对应传输报文的字符串
...
void HttpData::seperateTimer()
{
    ...
    // 每个HttpData 都有一个TimerNode， 时间节点
    // seperateTimer 可以将SPHttpData.reset() ， 它的对象会变为空。
    // timer_.reset() 会将其中的对象清空
    if (timer_.lock())
    {
        shared_ptr<TimerNode> my_timer(timer_.lock());
        my_timer->clearReq();
        timer_.reset(); //这个是weakptr的reset()
    }
}
...
void HttpData::handleRead()
{
    ...
    // do while(false)  只执行一次， 为什么使用 do while(false)
    do
    {
        bool zero = false;
        ...
        // 通过state_ 的状态转移进行变化。  ProcessState state_;
        if (state_ == STATE_PARSE_URI)
        {
            URIState flag = this->parseURI();
            if (flag == PARSE_URI_AGAIN) // 如果找不到 '\r' 会引发PARSE_URI_AGAIN, EAGAIN 的意思就是让你待会再试。 AGAIN
                break;
...
{
    // 如果调用handleConn, 因为定时器是用来去除那些太久没有连接的连接
    seperateTimer();
    __uint32_t &events_ = channel_->getEvents();
    if (!error_ && connectionState_ == H_CONNECTED)
    {
        if (events_ != 0)
        {
            ...
            //同时有输入和输出events， 将events保留epollout
...
URIState HttpData::parseURI()
{
    ...
    string cop = str;
    // 读到完整的请求行再开始解析请求
    // stirng::npos 如果str.find() 找不到会返回npos， 并且npos是 static const size_type npos = -1;
    // 然后发现只有使用int的时候， 它才会变为-1
    ...
    pos = request_line.find("/", pos); // 寻找报文
    if (pos < 0)
    { // 请求行找不到"/" , 然而却不会发送报文？尽管是成功解析
        fileName_ = "index.html";
        HTTPVersion_ = HTTP_11;
        // cout << "PARSE URI SUCCESS";
        return PARSE_URI_SUCCESS;
    }
    else
    {
        size_t _pos = request_line.find(' ', pos); // 定位pos_ 为文件目录的末尾space所在之处
        if (_pos < 0)
            return PARSE_URI_ERROR;
        else
        {
            if (_pos - pos > 1)
            {
                fileName_ = request_line.substr(pos + 1, _pos - pos - 1); // 将filename 解析出来
                size_t __pos = fileName_.find('?');                       // 有可能是Post请求， 带有 ? 号
                if (__pos >= 0)
                {
