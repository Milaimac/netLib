...
// g_output g_flush g_logTimeZone 这三个是线程全局变量
...
void Logger::Impl::formatTime(){
    ...
    if(seconds != t_lastSecond){
        ...
        struct tm tm_time;
        if(g_logTimeZone.valid()){      //g_logTimeZone 大可删掉
        ...
        // t_time 线程全局变量
        // 4+2+2+1+2+2+2 = 17
        int len = snprintf(t_time, sizeof(tm_time), "%4d%02d%02d %02d:%02d:%02d", tm_time.tm_year, tm_time.tm_mon, tm_time.tm_mday, tm_time.tm_hour, tm_time.tm_min, tm_time.tm_sec);
        assert(len == 17);
        (void) len;
    }
